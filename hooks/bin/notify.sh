#!/usr/bin/env bash
# workbench Notification hook → outbound Telegram ping, so you're nudged even when not
# watching the chat. Fires on permission_prompt / idle_prompt. No-ops unless this is
# a workbench project with remote != off AND Telegram credentials are present. The bot
# token + chat id live in ~/.claude/channels/telegram/.env (TELEGRAM_BOT_TOKEN,
# TELEGRAM_CHAT_ID) — NEVER in git. WORKBENCH_NOTIFY_DRYRUN=1 prints the would-be send
...
text="[${name:-workbench}] a session needs your attention (permission/idle). Check in or reply in chat."
