#!/usr/bin/env bash
# initlab PreToolUse safety guard. When remote control is enabled (remote != off),
...
# an initlab project with remote != off. Precise with python3; raw-scan fallback
...
input="$(cat)"
cmd=""
if [ -z "${INITLAB_GUARD_NO_PYTHON:-}" ] && command -v python3 >/dev/null 2>&1; then
...
block() { echo "initlab remote-guard: blocked — $1. If truly intended, run it locally with explicit approval (this guard protects remote-driven sessions)." >&2; exit 2; }
