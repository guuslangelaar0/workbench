...
bash "$HERE/scripts/init.sh" --level fleet --name "Override" --mission "x" --target "$TMP" >/dev/null 2>&1
...
TMP2="$(mktemp -d)"
bash "$HERE/scripts/init.sh" --level fleet --name "Fresh" --mission "x" --target "$TMP2" >/dev/null 2>&1
