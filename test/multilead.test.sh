#!/usr/bin/env bash
# Multi-teamlead + coordination: behavioral (wb-coord claims query, locks gitignore)
...
WB_SID_OVERRIDE=sessAstorage bash "$BBC" ping "lead:storage" >/dev/null 2>&1
WB_SID_OVERRIDE=sessAstorage bash "$BBC" claim "task:0042" >/dev/null 2>&1
WB_SID_OVERRIDE=sessBmobile  bash "$BBC" ping "lead:mobile"  >/dev/null 2>&1

out="$(WB_SID_OVERRIDE=sessBmobile bash "$BBC" claims task:0042 2>/dev/null)"
chk "another session sees the claim (text)"  "printf '%s' \"\$out\" | grep -q 'task:0042'"
chk "claims query exits 0 when claimed"      "WB_SID_OVERRIDE=sessBmobile bash '$BBC' claims task:0042 >/dev/null 2>&1"
chk "claims query exits 1 when free"         "! WB_SID_OVERRIDE=sessBmobile bash '$BBC' claims task:9999 >/dev/null 2>&1"
chk "claims query ignores own claim"         "! WB_SID_OVERRIDE=sessAstorage bash '$BBC' claims task:0042 >/dev/null 2>&1"

st="$(WB_SID_OVERRIDE=sessBmobile bash "$BBC" status 2>/dev/null)"
...
WB_SID_OVERRIDE=sessOld bash "$BBC" claim "task:0050" >/dev/null 2>&1
chk "claim inserts claims field when absent" "grep -q '\"claims\":\"task:0050\"' '$TMP/.claude/locks/sessions/sessOld.json'"
chk "another session sees the inserted claim" "WB_SID_OVERRIDE=sessXother bash '$BBC' claims task:0050 >/dev/null 2>&1"
...
chk "teamlead pings coord"               "grep -q 'wb-coord' '$C/teamlead.md'"

# --- presence: dispatch now claims; orchestration composes coordination ---
chk "dispatch claims the task"           "grep -q 'wb-coord' '$C/dispatch.md' && grep -q 'claim' '$C/dispatch.md'"
