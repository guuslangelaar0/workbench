...
(cd "$TMP" && git init -q)
bash "$HERE/scripts/init.sh" --level fleet --name "Uninstall" --mission "x" --target "$TMP" >/dev/null 2>&1
...
TMP2="$(mktemp -d)"
bash "$HERE/scripts/init.sh" --level fleet --name "Edited" --mission "x" --target "$TMP2" >/dev/null 2>&1
...
TMP3="$(mktemp -d)"
bash "$HERE/scripts/init.sh" --level fleet --name "Data" --mission "x" --target "$TMP3" >/dev/null 2>&1
