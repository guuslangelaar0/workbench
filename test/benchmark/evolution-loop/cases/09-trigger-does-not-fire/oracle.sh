...
set -uo pipefail
snap_backlog="$(cat .claude/admin-evolution/.snapshot-backlog-count 2>/dev/null || echo 0)"
snap_ledger="$(cat .claude/admin-evolution/.snapshot-ledger-lines 2>/dev/null || echo 0)"
...
now_ledger="$(wc -l < .claude/admin-evolution/ideas-log.md | tr -d ' ')"
