...
chk "release gate dry-run names live gates" "printf '%s' \"\$DRY_LIVE\" | grep -q 'test/e2e/run.sh' && printf '%s' \"\$DRY_LIVE\" | grep -q 'WB_BENCH=1' && printf '%s' \"\$DRY_LIVE\" | grep -q 'WB_BENCH_ATTEMPTS=2' && printf '%s' \"\$DRY_LIVE\" | grep -q 'bench-intents.sh'"
