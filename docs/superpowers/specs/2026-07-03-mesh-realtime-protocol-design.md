...
- New `workbench-mesh watch --as ACTOR` connector: a persistent Rust WS client that ACKs message delivery at wire speed, independent of whether the actor's Claude Code session is awake, and wakes the session via the existing inbox-file mechanism.
...
- **`message.delivered`** — repurposed (was dead/unused). Payload: `{ack_of: <seq>}`. Emitted by the addressee's `watch` connector the instant the frame lands on its socket — before the Claude session is necessarily awake.
...
- On WS connect, the client sends a `{"type":"subscribe","rooms":[...]}` control frame (dashboard: rooms currently rendered; `watch` connector: the actor's own inbox + every room the actor is a member of, from `room.member_added` history). The server always includes the actor's own unicast channel regardless of what's requested.
...
## Connector process (`workbench-mesh watch`)

New subcommand: `workbench-mesh watch --as ACTOR [--target <path>]`. A persistent Rust WS client (not the bash+python poll loop it replaces) that:
1. Connects and sends the actor's subscribe frame (own inbox + member rooms).
2. On any inbound frame addressed to the actor (message.sent, task.handoff, mention, etc.), immediately emits `message.delivered{ack_of: seq}` and writes to the actor's inbox file — the same wake mechanism `mesh-inbox-wait.sh` uses today, just event-driven instead of polled, collapsing the wake latency from up to 500ms to wire-speed.
3. Uses native WS ping/pong (available to a Rust client, unlike browser JS) to measure and report connector↔server RTT.
4. Stays running for the lifetime of the actor's dev session; `serve --as` continues to be how the mesh server itself starts, this is a separate always-on client process per actor.

This subsumes `mesh-inbox-wait.sh`'s role; that script is retired once `watch` ships (kept as a fallback path only if a `watch` binary isn't available, e.g. before a rebuild).
...
- A message stuck at "server-received" (no `message.delivered` within ~30s) surfaces a subtle "no response yet" hint — signals the addressee's `watch` connector is down, not merely that a human is slow.
...
- `watch` connector: uses native WS ping/pong directly (available to a Rust client below the browser sandbox), same ~5s cadence.
...
- `watch` connector process dies: no `message.delivered` ever arrives, surfaced via the 30s "no response yet" hint above; the inbox-file wake mechanism it would have triggered simply doesn't fire, so the underlying Claude session degrades to needing a manual nudge — same failure mode as today, not worse.
...
- `watch` connector: e2e test measuring ack latency from event-post timestamp to `message.delivered` receipt, asserting sub-100ms on localhost.
