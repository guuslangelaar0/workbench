...
2. **Budget signal source** — does Claude Code expose exact per-turn token/cost usage to a plugin (OTel/telemetry), or do we estimate? Determines whether SQ-3 is exact or honest-estimate. *Needs a quick probe before building SQ-3.*
