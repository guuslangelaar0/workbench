...
**Status:** Implemented 2026-06-29 — BM-1..BM-8 (all). The intent-conformance benchmark (BM-8) is the primary one; BM-6 (knob search) turns it into an optimizer. Conformance fixture widened to 11 cases with a train/holdout split (anti-overfit). 50 test suites green. Live conformance history: 2/5 → 5/5 (first description fix) → on the widened set the full live run scored **9/11**, and **both failures were held-out cases** (the Goodhart guard catching what the train-tuned descriptions didn't generalize): `in-review cap not checked before pulling new work`, and `a "plan a big multi-part effort" intent at solo not landing as tracked backlog tasks`. Two routing-table rows fixed both; re-validated live → **11/11**. Then (CB-6) the holdout was replenished: 09/11 rotated into train and three genuinely-unseen cases (`12-deps-blocked`, `13-security-bug`, `14-budget-spend`) became the new holdout — all testing behaviors the routing table doesn't mention; **all passed live 3/3**. Fixture is now **14 cases (11 train + 3 holdout)**; the holdout is a clean anti-overfit guard again (§6c).
**Date:** 2026-06-29
**Owner:** Guus
...
**Follow-up**
- [x] **CB-6 — Replenish the holdout.** The fixed-against `09`/`11` were rotated **into train**, and three genuinely-unseen cases became the new holdout: `12-deps-blocked` (a task blocked by an unfinished dependency — don't blindly start it), `13-security-bug` (a plaintext-password finding is auto-filed as a tracked bug), `14-budget-spend` (a "how much have we spent" intent routes to `/workbench:budget`). None are in the intent-routing table, so they test whether the **command descriptions alone** carry the intent. All three **passed live 3/3** — the holdout is a clean anti-overfit signal again. Fixture is now 14 cases (11 train + 3 holdout).
...
This rule was then applied (CB-6): 09/11 moved into train, three unseen cases (`12-deps-blocked`, `13-security-bug`, `14-budget-spend`) became the new holdout — all chosen to test behaviors the **intent-routing table does not mention**, so they probe whether the command descriptions alone carry the intent. They passed live 3/3 on the first try, so this round the descriptions generalized; the holdout is clean again.
