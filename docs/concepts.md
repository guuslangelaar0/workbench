...
**Opt-in replenishment: evolution summits.** A project can give the loop a second replenishment engine by creating a persona roster at `.workbench/evolution/personas.json` (via `/workbench:evolve init` — N generators + exactly one critic, sized to your level). The loop then consults `scripts/evolve.sh check` in its rotation: when the unblocked backlog runs below the roster's low-water mark, or more than its cadence (default 24h) has elapsed since the last summit, it convenes a **summit** — parallel generator personas propose new ideas *and* retrospectively re-audit a rotating slice of verified/shipped work, a single critic pass filters, and approved survivors become normal backlog tasks. Every idea is recorded in the append-only ideas ledger (`.workbench/evolution/ideas-log.md`), which doubles as the retrospective-coverage record and the human's skimmable audit trail. Creating the roster is the human's *standing approval* for panel-sourced tasks, so the features-only-suggest rule above stays intact; no roster, no summits. See [commands.md](commands.md) → `/workbench:evolve`.
...
| LAN | `/workbench:mesh start --lan` | Explicit opt-in for the local network; another machine joins with a scoped invite token from `/workbench:mesh invite` and `/workbench:mesh connect HOST TOKEN --fingerprint sha256:HASH [DEVICE]`, confirming the printed fingerprint code. |
| Public | Deferred | Public internet exposure is out of scope in this version. |

When started for LAN, the wrapper prints every address form a user may need to copy: the host name, the `.local` mDNS name, the raw LAN IP, and the port, all over TLS. Trusted LAN URL acceptance is supported through `connect`, with tokens passed as command arguments rather than URL parameters. Every LAN/remote connection pins the host's self-signed certificate by fingerprint: `invite` prints ready-to-paste `connect` commands carrying `--fingerprint sha256:HASH` plus a human-checkable "Fingerprint code: NNN-NNN"; `connect` requires that flag (plain `http://` is rejected) and, run interactively without `-y`/`--yes`, asks you to confirm the code before trusting the connection; `status`/`who` reprint the same code afterward so it can be re-checked at any time. Ignored runtime metadata is also written to `.workbench/mesh/server.json`, so `/workbench:mesh open` can print the current command-center URL later. That file is for host/port discovery and may include an ephemeral local daemon access token, but it is not the durable same-user credential store or root key and the wrapper does not print it in invite/open URLs.
