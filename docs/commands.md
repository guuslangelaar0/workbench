...
Move an explicit unblocked task to `in-development/` and dispatch it to a native Claude engineer lane — the default engine-specific implementation `/workbench:dispatch` routes to when no `--engine` flag is given. Claims via `wb-coord`, resolves the model via the `models` skill, and spawns the `engineer` Task-tool lane: foreground by default, a native `--worktree` (optionally `--background` or `--wait`), or `--shared` to skip a persistent worktree. `--worktree` with `--wait` (no `--background`) creates the worktree but runs the engineer synchronously in the foreground, blocking until it completes. Workbench still owns task claiming, lifecycle, review, and `/workbench:verify`.
...
Start the command center on the local network for another trusted machine on the same LAN, over TLS with a self-signed, pinned certificate. The output shows the hostname form, the `.local` mDNS form, the raw LAN IP, and the port, then keeps public internet exposure unavailable in this version. Use `/workbench:mesh invite --role worker --ttl-seconds 900` to create a scoped invite token.

### `/workbench:mesh invite [--role ROLE] [--ttl-seconds N] [--max-uses N]`
Create a LAN invite token. Tokens are the auth boundary for another machine joining the mesh; local same-user access uses durable credentials stored under `$WORKBENCH_HOME` / `~/.workbench`, not project-local `.workbench/mesh/server.json`. `ROLE` defaults to `worker`. For LAN mode, the output also prints ready-to-paste `connect`/`connect-host`/`connect-ip`/`connect-url` commands with `--fingerprint sha256:HASH` filled in, plus a human-checkable "Fingerprint code: NNN-NNN" derived from the same pinned certificate — read that code aloud/compare it out-of-band with whoever is running `connect` as a tamper check on the invite string itself.

### `/workbench:mesh connect [HOST|URL] TOKEN [DEVICE] --fingerprint sha256:HASH [-y|--yes]`
Accept an invite token for this device. Without a host/URL, the token is redeemed against the local project runtime. With a bare host (`HOST` or `HOST:PORT`, defaulting to the LAN port) or an `https://` URL, the token is redeemed against a trusted LAN/remote mesh host and the joining device stores its credential under `$WORKBENCH_HOME/mesh/` — plain `http://` is rejected outright. Any LAN/remote connect requires `--fingerprint sha256:HASH`, copied verbatim from what `invite` printed; there is no unpinned fallback. Unless `-y`/`--yes` is passed, running from an interactive terminal prints the fingerprint code and asks you to confirm it matches what the host showed before the connection proceeds (skipped automatically when stdin isn't a TTY, e.g. in scripts) — this catches a tampered invite string, which the TLS pin alone cannot. The token is a command argument, not a URL parameter.
...
Inspect the mesh runtime. `status` and `who` show registered actors and rooms, and both also reprint the "Fingerprint code: NNN-NNN" line for LAN/remote mode so you can re-check it any time after `invite`, not just when the invite was created (local mode has no pinned certificate, so the line is omitted). `jobs` lists active Workbench jobs when Codex lanes are in flight and otherwise falls back to recent mesh job/task events, and `open` prints the cached non-tokenized command-center URL from ignored discovery metadata in `.workbench/mesh/server.json`.
