---
name: codex-bridge
description: Use when coordinating with Codex (a second coding agent) in an initlab project — the shared disk-based protocol, codex:rescue, and codex:setup.
---

# Codex bridge

Claude and Codex can work the same initlab project. Disk is the shared source of truth — Codex can't see Claude's in-memory team state, and vice versa.
...
- **Ownership:** before editing, check `git status` + the task file; claim via the task's `## Notes` owner line and `bb-coord claim`. Keep commits scoped so parallel agents don't sweep each other.
