...
2.7. **Global regression gate.** A task can pass its own narrow check and still break something else. Before advancing, run the project's FULL suite:
   `bash "${CLAUDE_PLUGIN_ROOT}/scripts/regression-gate.sh" --target "${CLAUDE_PROJECT_DIR}"`
   It runs every command in `project.checks` (config), compares against the last known-green run to flag **was-green-now-red**, and is level-scaled (blocks with exit 3 at `crew`/`fleet`, advisory at `solo`/`pair`; skips cleanly if no checks are configured). On red it files a `warn` suggestion. If it blocks, **bounce the task to `in-development`** and — since a regression is a bug — auto-file a `[P0]` bug task for the failing check, then fix before re-verifying.
