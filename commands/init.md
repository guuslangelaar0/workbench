...
Steps:
1. Parse `$ARGUMENTS`. If `--name` is not present (or the command was invoked bare), don't fail or dump usage — run a short wizard: use AskUserQuestion for the project name (offer the repo directory name as the default candidate), optionally a one-line mission and launch target, then the level per step 2; confirm the assembled `init.sh` invocation before running it.
