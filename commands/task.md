...
3. Report the created path + ID, and remind the user the task starts in `backlog/` — fill in `## Why`, `## Acceptance criteria`, and `**Verification:**`, then `/initlab:dispatch <id>` to start it. See the `task-lifecycle` skill for the format.
